...
const RoadmapTab: React.FC<Props> = ({ data, onUpdate }) => {
  ...
  return (
    <div className="h-[68dvh] sm:h-[420px] w-full overflow-x-auto p-1 sm:p-2 custom-scrollbar scroll-smooth">
      <DragDropContext onDragEnd={onDragEnd}>
        <div className="flex gap-3 h-full min-w-max pb-2">
          {board.columnOrder.map((colId, idx) => {
            const column = board.columns[colId];
            return (
              <div key={column.id} className="w-[78vw] max-w-[240px] sm:w-[220px] md:w-[240px] flex flex-col h-full max-h-full bg-[#0a0a0a] border border-white/[0.06] rounded-xl overflow-hidden shrink-0">
